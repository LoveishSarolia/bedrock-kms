 ...
 * @return {Promise<Object>} resolves to the database record.
 */
api.update = async ({config, allowControllerUpdate = false} = {}) => {
  assert.object(config, 'config');
  assert.string(config.id, 'config.id');
  assert.string(config.controller, 'config.controller');
  assert.number(config.sequence, 'config.sequence');
  assert.bool(allowControllerUpdate, 'allowControllerUpdate');
  ...
  //update controller only if allowControllerUpdate and flag is true
  if(allowControllerUpdate) {
    set.controller = database.hash(controller);
    delete query.controller;
  }
  const result = await database.collections.kmsKeystore.update(query, {
    $set: set
